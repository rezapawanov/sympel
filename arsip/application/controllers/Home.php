...
class Home extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('hak_akses') != "") {
			$d['judul'] = "Dashboard";
			$hitung_dokumen = $this->db->query("SELECT COUNT(*) as hitung FROM dokumen")->row();
			...
			$d['hitung_dokumen'] = $hitung_dokumen->hitung;
			$d['hitung_rak'] = $hitung_rak->hitung;
			$d['hitung_box'] = $hitung_box->hitung;
			$d['hitung_map'] = $hitung_map->hitung;
			$d['data_jenis'] = $this->db->query("SELECT * FROM mst_jenis_dokumen");
