                ...
                <div class="row">
                    <div class="col-4">
                        <div class="form-group">
                            <label for="nis">No Induk / Nama Siswa</label>
                            <select name="nis" id="nis" class="form-control form-control-lg"></select>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            type: "GET",
            url: BASE_URL+"siswa/getAll",
            data: {},
            dataType: "JSON",
            success: function (response) {
                $.each(response, function (i, val) { 
                    $('select[name="nis"]').append(`<option value="${val.id_siswa}">${(val.nis) ? val.nis : ''} - ${val.nama_siswa}</option>`);
                });
            }
        });
