...
class Pengguna extends CI_Controller {
	...
	public function guru_save() {
			...
			if($tipe == "add") {
				...
				$cek2 = $this->db->query("SELECT nik FROM mst_guru WHERE nik = '$in[nik]'");
				if($cek->num_rows() > 0) { 
				...
				$cek = $this->db->query("SELECT nik FROM mst_guru WHERE nik = '$in[nik]' AND id_guru != '$where[id_guru]'");
				$cek2 = $this->db->query("SELECT nik FROM mst_guru WHERE nip = '$in[nip]' AND id_guru != '$where[id_guru]'");
				if($cek->num_rows() > 0) { 
					$this->session->set_flashdata("error","Gagal Input.  NIK Sudah Digunakan");
