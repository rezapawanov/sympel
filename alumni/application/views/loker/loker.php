          ...
          <div class=" col-12">
            <div class="card card-purple card-outline">
              <div class="card-header">
                <a class="btn bg-purple btn-sm" href="<?php echo base_url(); ?>loker/loker_tambah"><i class="fa fa-plus"> </i> Tambah Data</a>
              </div>
              <!-- /.card-header -->
              <div class="card-body table-responsive p-2">
                <table id="datatb" class="table table-bordered table-hover table-striped">
                  ...
                  <tbody>
                    <?php
                    $no = 1;
                    foreach ($loker->result_array() as $data) { ?>
                      <tr>
                        <td><?php echo $no; ?></td>
                        <td><?php echo $data['judul_loker']; ?></td>
                        <td><?php echo date("d-m-Y H:i", strtotime($data['tanggal'])); ?></td>
                        <td style="text-align:center;width:80px;">
                          <a class="btn btn-primary btn-xs" href="<?php echo base_url() . 'loker/loker_edit/' . $data['id_loker']; ?>"><i class="fa fa-edit"> </i> </a> |

                          <a class="btn btn-danger btn-xs" href="<?php echo base_url() . 'loker/loker_hapus/' . $data['id_loker']; ?>" onclick="return confirm('Yakin ingin hapus data ? ');"><i class="fa fa-trash"> </i> </a>
                        </td>
                      </tr>
                    <?php $no++;
                    } ?>
