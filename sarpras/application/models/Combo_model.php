...
class Combo_model extends CI_Model {
	...
	public function combo_jenis_barang($id="") {
		$hasil = "";
		$q = $this->db->query("SELECT id_jenis_barang,nama_jenis_barang FROM mst_jenis_barang ORDER BY id_jenis_barang ASC");
		foreach($q->result() as $h) {
			if($id == $h->id_jenis_barang) {
				$hasil .= '<option value="'.$h->id_jenis_barang.'" selected="selected">'.$h->nama_jenis_barang.'</option>';
			} else {
				$hasil .= '<option value="'.$h->id_jenis_barang.'">'.$h->nama_jenis_barang.'</option>';
			}
		}
		return $hasil;
	}
