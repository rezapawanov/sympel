...
class Laporan extends CI_Controller
{
	...
	public function proses_tampil_arsip()
	{
		...
		if (empty($this->input->post("jenis_barang"))) {
			$jenis_barang = 'all';
		} else {
			$jenis_barang = $this->input->post("jenis_barang");
		}

		redirect("laporan/arsip/" . $tgl_awal . "/" . $tgl_akhir . "/" . $id_ruangan . '/' . $id_lemari . '/' . $id_rak . '/' . $id_box . '/' . $id_map . '/' . $id_urut . '/' . $jenis_barang);
	}
	...
	public function arsip($tgl_awal = "", $tgl_akhir = "", $id_ruangan = "", $id_lemari = "", $id_rak = "", $id_box = "", $id_map = "", $id_urut = "", $jenis_barang = "")
	{
		...
		if (!empty($tgl_awal) && !empty($tgl_akhir)) {
			...
			$d['laporan_arsip'] = $this->Laporan_model->arsip($tgl_awal, $tgl_akhir, $id_ruangan, $id_lemari, $id_rak, $id_box, $id_map, $id_urut, $jenis_barang);
		} else {
			$d['laporan_arsip'] = "";
