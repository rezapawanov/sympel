...
class Pelanggaran_siswa_model extends CI_Model {
	...
	public function pelanggaran_siswa($tahun_ajaran) {
		$q = $this->db->query("SELECT * FROM vw_pelanggaran_siswa WHERE tahun_ajaran = '$tahun_ajaran' ORDER BY id_pelanggaran_siswa DESC");
		return $q;
	}

	public function pelanggaran_siswa_edit($id) {
		$q = $this->db->query("SELECT * FROM vw_pelanggaran_siswa WHERE id_pelanggaran_siswa = '$id'");
		return $q;
	}
